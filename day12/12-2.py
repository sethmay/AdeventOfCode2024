...
def trace(plots, pos, side, start_pos):
    side_count = 0
    plot = plots[pos]
    print (start_pos, pos, side, plot.n, plot.s, plot.e, plot.w)

    if (pos == start_pos and side == 'w'):
        print('looped')
        return side_count + 1

    #print(side)
    if side == 'n':
        #print('side = n', plot.n, (pos[0], pos[1]+1), (pos[0], pos[1]+1) in plots)
        if (pos[0], pos[1]+1) in plots:
            side_count += trace(plots, (pos[0], pos[1]+1), side, start_pos)
        side = 'e'

    #print(side)
    if side == 'e':
        #print('side = e', plot.n, (pos[0]+1, pos[1]), (pos[0]+1, pos[1]) in plots)
        if (pos[0]+1, pos[1]) in plots:
            side_count += trace(plots, (pos[0]+1, pos[1]), side, start_pos)
        side = 's'
...
def count_sides(regions):
    ...
    for region in regions:
        plots = region.plots
        first_plot_key = next(iter(plots))
        first_plot = plots[first_plot_key]
        start_pos = (first_plot.row, first_plot.col)
        #print (start_pos)

        region.sides = trace(plots, start_pos, 'n', start_pos)
        total_sides += region.sides
        #print (region.sides)
        break
    return total_sides
...
def solve(input_data):
    ...
    total_sides = count_sides(regions)
    print (total_sides)
