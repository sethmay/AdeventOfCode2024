...
def calculate_antinode(node1, node2, dist = 1):

    distance_row = abs(node1[0] - node2[0]) * dist
    distance_col = abs(node1[1] - node2[1]) * dist
    antinode_row = 0
    antinode_col = 0
    if (node1[0] < node2[0]):
...
def find_antinodes (map, nodes):
    ...
    while len(temp_nodes) > 0:
        ...
        while j < len(temp_nodes):
            ...
            antinode1 = calculate_antinode(node, temp_node, 1)
            antinode2 = calculate_antinode(temp_node, node, 1)
            m = 2
            while is_node_inbounds(map, antinode1):
                antinodes.append(antinode1)
                antinode1 = calculate_antinode(node, temp_node, m)
                m += 1

            n = 2
            while is_node_inbounds(map, antinode2):
                antinodes.append(antinode2)
                antinode2 = calculate_antinode(temp_node, node, n)
                n += 1
...
def remove_antinode_collisions (map, antinodes_list):
    ...
    row = 0
    while row < len(temp_map):
        col = 0
        while col < len(temp_map[0]):
            if not temp_map[row][col] == '.':
                antinode_result.append([row, col])
            col += 1
        row += 1
